...
class DatabaseInterface:
    def __init__(self, engine):
        self._session = self.create_database_session(engine=engine)
    ...
    def commit_email_to_table(self, df):
        email_id_lst, recipe_id_lst = self.build_email_index(df=df, colum_to_index="to")
        self._session.add_all(
            [
                Email_to(email_id=email_id, email_to=email_to)
    ...
    def commit_email_from_table(self, df):
        email_id_lst, recipe_id_lst = self.build_email_index(
            df=df, colum_to_index="from"
        )
        self._session.add_all(
            [
                Email_from(email_id=email_id, email_from=email_from)
    ...
    def build_email_index(self, df, colum_to_index):
        email_id_lst, column_id_lst = [], []
        for eid, email_lst in zip(df["id"], df[colum_to_index]):
            email_split_lst = email_lst.split(", ")
            for email_add in [
                self.get_email_address(email=email) for email in email_split_lst
            ]:
